...
| [`convertGrpcStatus`](src/main/kotlin/com/engine/protoc/openapi/ProtocGenOpenAPI.kt#L305) | boolean | `false` | Add a reusable `google.rpc.Status` schema and a `"default"` response entry referencing it on every operation. Mirrors Envoy's `convert_grpc_status`. |
