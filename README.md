...
| [`referenceLinkTarget`](src/main/kotlin/com/engine/protoc/openapi/ProtocGenOpenAPI.kt#L437) | enum | `SWAGGER_UI` | Renderer dialect that CommonMark reference links (`[Type]`, `[Service.Method]`) in `description` fields resolve to: `SWAGGER_UI` (operations + tags), `REDOC` (operations + tags + generated schema sections), or `NONE`. Unresolved references are stripped of their brackets and rendered as an inline code span (`` `Property` ``), with a warning. |
