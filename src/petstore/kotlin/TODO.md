This document lists mismatches between the compiled openapi spec vs the openapi reference spec found at petstore.openapi.modified.json

# Missing from generated output (expected but actual: null):
-[x] $.info.x-namespace — "swagger" not emitted
-[x] $.paths./pet.put/post request body schema.$ref — "#/components/schemas/Pet" missing (5 occurrences)
-[x] $.paths./pet/{petId} — entire path missing (generated as /pet/{value} instead — diff 8)
...
# Wrong values:
-[x] $.paths./pet.post.responses.200.content.application/json.schema.description — "A Pet in JSON format" vs "A Pet in JSON Format" (capitalization)
